// Write-Ahead Logging (WAL) — how a database survives a crash mid-transaction (the core of
// ARIES). The rule: before a change touches a data page, its log record is appended to a
...
export function recover(log: Rec[], initial: Record<string, string>): Recovery {
  const committed = new Set(log.filter((r) => r.type === 'commit').map((r) => r.txid));
  const updates = log.filter((r): r is Extract<Rec, { type: 'update' }> => r.type === 'update');
