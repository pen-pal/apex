...
// true trig, max error ~1.2e-4 over ±80° in 14 iterations. It's in calculators, FPGAs, GPUs, DSPs. Sandboxed/CONCEPTUAL.
  ...
  return (
    <GuidedStory
      ...
      explain={{
        ...
        takeaway: <>Start with a vector on the x-axis and rotate it to the target by a fixed sequence of micro-rotations, each by ±atan(2⁻ⁱ). Those angles are special because a rotation by an angle with tan = 2⁻ⁱ works out to <code>x′ = x ∓ (y ≫ i)</code>, <code>y′ = y ± (x ≫ i)</code> — a bit-shift and an add, no multiply. At each step you rotate positive if you’re short of the target and negative if you’ve overshot, and because the angles halve (atan 1, atan ½, atan ¼, …) it’s a binary search in angle space that pins θ in a dozen or so steps. Each rotation also scales the vector by a known factor, so the whole chain multiplies its length by a fixed constant <strong>K ≈ 0.6073</strong>; you cancel it by starting the vector at length K, and the final coordinates come out as exactly cos θ and sin θ (verified to four digits). The same shift-and-add engine in other modes yields atan and vector magnitude, and its hyperbolic cousin gives log, exp, and square root — one tiny circuit for a whole math library, which is why CORDIC is in calculators, FPGAs, GPUs, DSP chips, and flew in the 1960s guidance computers.</>,
      }}
