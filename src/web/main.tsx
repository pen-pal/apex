...
import { DelaunaySection } from './DelaunaySection';
import { MontgomerySection } from './MontgomerySection';
import { TwoSatSection } from './TwoSatSection';
import { PerlinSection } from './PerlinSection';
import { PoissonSection } from './PoissonSection';
    ...
    <div className={`app ${activePath ? 'has-jbar' : ''}`}>
      ...
      <main className="content">
        ...
        {section === 'delaunay' && (
          <>
            <header>
              <h1>Delaunay triangulation — fat triangles, no slivers</h1>
              <p className="sub">Connect scattered points into triangles that avoid slivers: the Delaunay triangulation maximizes the smallest angle, giving triangles as close to equilateral as the points allow — essential for finite-element meshes, terrain, and interpolation. Its defining rule is the empty-circumcircle property (no point inside any triangle's circumcircle), and it's the exact dual of the Voronoi diagram. Built here with Bowyer-Watson. Click to add points; toggle the Voronoi dual to see the connection.</p>
            </header>
            <DelaunaySection />
          </>
        )}
        ...
        {section === 'montgomery' && (
          <>
            <header>
              <h1>Montgomery multiplication — modular multiply without division</h1>
              <p className="sub">RSA and elliptic-curve crypto compute a·b mod n for a huge modulus thousands of times, and the slow, timing-leaky part is the "mod n" — a full division. Montgomery's trick works in a coordinate system where numbers are held as a·R mod n, so reduction (REDC) needs only multiplies, adds, a bit-mask (mod R) and a shift (÷R) — never a division by n, and the same fixed operations regardless of the values (constant-time). Enter a modulus and two numbers and watch a·b mod n fall out with only masks and shifts.</p>
            </header>
            <MontgomerySection />
          </>
        )}
        ...
        {section === 'twosat' && (
          <>
            <header>
              <h1>2-SAT — boolean satisfiability in linear time</h1>
              <p className="sub">General SAT is NP-complete, but restrict every clause to two literals and it becomes solvable in linear time by a beautiful reduction to graph theory. A clause (a ∨ b) is two implications (¬a ⇒ b, ¬b ⇒ a); build the implication graph and the formula is unsatisfiable exactly when some variable and its negation land in the same strongly-connected component. One SCC pass decides it and builds a satisfying assignment. Pick a formula and watch the components and verdict.</p>
            </header>
            <TwoSatSection />
          </>
        )}
        ...
        {section === 'poisson' && (
          <>
            <header>
              <h1>Poisson-disk sampling — evenly-spaced randomness</h1>
              <p className="sub">Scatter points that look random but never clump: every pair at least r apart, yet the area stays evenly covered. This "blue noise" is what rendering, stippling, and procedural scattering want — uniform random points bunch up and leave holes, causing splotchy shadows and grainy antialiasing. Bridson's algorithm builds it in O(n) by firing candidate darts into the ring between r and 2r around active points, with an O(1) grid spacing-check. Flip blue vs white noise and watch the min distance.</p>
            </header>
            <PoissonSection />
          </>
        )}
