...
export function SyscallSection() {
  ...
  return (
    <GuidedStory
      scenes={scenes}
      explain={{
        idea: <>Your program runs walled off in “user mode,” unable to touch the disk, the network, or another process’s memory directly — that isolation is what stops one buggy or malicious app from taking down the whole machine. So to do anything real, it has to ask the <strong>kernel</strong> (the core of the operating system — the one piece of code allowed to touch the hardware directly), and a <strong>system call</strong> is the single carefully controlled doorway through the wall. This steps one read() across into the kernel and back — and that wall is the line half of the attacks later on are trying to cross.</>,
        takeaway: <>The program loads a syscall number and arguments into registers and runs one special instruction that traps into the kernel; the CPU switches to privileged mode, jumps to a fixed entry point the kernel installed, validates the arguments, does the privileged work, and returns. The doorway is deliberately narrow and one-way, so user code can <em>request</em> privileged work without ever <em>gaining</em> privilege. Each crossing costs a mode switch of hundreds of nanoseconds — which is exactly why batching syscalls (io_uring) and avoiding them (a futex’s userspace fast path) are worth real engineering effort. This wall is also the whole game in system security: because user code can <em>request</em> privileged work but never <em>seize</em> privilege, a huge class of attacks reduces to one goal — <strong>trick the kernel into doing privileged work it shouldn’t</strong> (a sandbox escape or privilege escalation), or read straight across the boundary through a CPU flaw (<strong>Meltdown</strong>, a few rungs on). Every argument the kernel validates at this doorway is a check an attacker probes for a gap.</>,
      }}
