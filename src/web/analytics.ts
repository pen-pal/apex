// Cookieless analytics for Apex, a static SPA hosted on GitHub Pages. Every provider is loaded as a plain
// <script> beacon (no cookies, no localStorage, no proxy, no consent banner). The app never changes the URL when
// you switch sections, so per-section popularity is tracked by firing a VIRTUAL PAGEVIEW on each section change
// (GoatCounter and Umami support this via their JS API). Cloudflare's free Web Analytics beacon records real
// navigations only, so it captures visits/referrers/geo but not per-section.
//
...
export function trackSection(id: string): void {
  ...
  const base = (import.meta as { env?: { BASE_URL?: string } }).env?.BASE_URL || '/'; // '/apex/' on GitHub Pages
  const path = `${base}${id}`;
