  ...
  return (
    <GuidedStory
      ...
      explain={{
        ...
        takeaway: <>It rests on a one-way function: easy forward, infeasible backward unless you know a secret. Multiplying two big primes is easy; factoring the product back into them is not — so the product can be public while the two factors stay private. Run the function one way and anyone can encrypt to you but only you can read; run it the other way and only you can sign but anyone can verify. Those two directions are exactly how a website proves its identity with a certificate and how the TLS handshake agrees on a key with a stranger. One assumption still hangs, though: that the padlock you grabbed really is the bank’s — if Eve can hand you <em>her</em> padlock instead, she reads everything (the <strong>man-in-the-middle</strong> attack), which is the whole reason <strong>certificates</strong> exist.</>,
      }}
