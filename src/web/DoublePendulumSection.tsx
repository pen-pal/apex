...
// (energy conserved to 9 digits, verified in node), yet two starts differing by a thousandth of a degree diverge
...
export function DoublePendulumSection() {
  ...
  const scenes: StoryScene[] = [
    ...
    scene('energy', 'Nothing is faked — energy is conserved', 'This isn’t numerical noise. Integrate the equations carefully and the total energy (kinetic + potential) holds constant to nine digits. Each pendulum is tracing its own exact trajectory perfectly — the two just happen to be different trajectories, because they started a hair apart.'),
  ...
  return (
    <GuidedStory
      ...
      explain={{
        ...
        takeaway: <>The double pendulum obeys a fixed pair of coupled differential equations from Newton’s laws; integrate them accurately (here with RK4) and total energy stays constant to nine decimal places — proof the motion is <em>exact</em>, not numerical noise. So it’s fully deterministic: the same initial angles and speeds always trace the same path. But it shows <strong>sensitive dependence on initial conditions</strong>: the gap between two nearly-identical starts grows roughly exponentially in time (a positive <strong>Lyapunov exponent</strong>), so a difference of 0.001° becomes a difference of 180° within seconds. Chaos is exactly this combination — deterministic yet unpredictable — because any real measurement has finite precision, and that tiny uncertainty is amplified without bound. It’s the same reason weather forecasts degrade to noise beyond about two weeks (Lorenz’s “butterfly effect”), why a specific die roll is unpredictable despite deterministic physics, and why chaotic dynamics show up from planetary orbits to heart rhythms. Determinism and predictability are not the same thing.</>,
      }}
    ...
    <svg viewBox="0 0 900 440" className="story-svg">
      ...
      <text x="450" y="420" className="dp-foot" textAnchor="middle">
        {on('what') ? 'exact, deterministic equations — same start, same motion'
          ...
          : on('energy') ? 'total energy constant to 9 digits — exact, not noise'
