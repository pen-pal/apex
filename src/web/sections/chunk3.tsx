...
import { EbpfSection } from '../EbpfSection';
import { HoneypotSection } from '../HoneypotSection';
import { PortKnockSection } from '../PortKnockSection';
import { CensorshipSection } from '../CensorshipSection';
import { EchSection } from '../EchSection';
...
export const chunk3: Record<string, SectionEntry> = {
  ...
  "ech": { Component: EchSection, title: <>ECH — Encrypted Client Hello</>, sub: <>HTTPS doesn’t hide <em>where</em> you go: TLS 1.3 still sends the server name (SNI) in the ClientHello in plaintext, so an ISP or national firewall blocks by name. ECH encrypts the real ClientHello under a key fetched from the server’s DNS record and shows only a shared cover name. But the SNI is one of three leaks — toggle ECH, private DNS, and a shared front, and watch the censor fall back to the plaintext DNS query or the destination IP. ECH only hides you when all three channels are closed.</> },
  "portknock": { Component: PortKnockSection, title: <>Port knocking</>, sub: <>A hidden service sits behind a firewall that drops everything; a daemon opens it only after it sees a secret sequence of connection attempts to closed ports — 7000 → 8000 → 9000, in order. Knock the ports yourself to open SSH, then run a blind port scan and watch it never stumble on the sequence. The honest caveat: it’s obscurity, not authentication — the knocks cross the wire in the clear.</> },
  ...
  "capabilities": { Component: CapabilitiesSection, title: <>Linux capabilities — least privilege vs the exploit</>, sub: <>“Root” isn’t one thing: the kernel splits it into ~40 <strong>capabilities</strong>, each a class of privileged action, and a process holds only the ones it’s granted. Coarser than seccomp’s syscall filter, and they compose. Take an exploited web server and toggle its capabilities: it needs only <code>CAP_NET_BIND_SERVICE</code> to bind :80, so dropping the rest means the attacker can’t read <code>/etc/shadow</code>, load a rootkit, or escape the container — while full root, or a careless <code>CAP_SYS_ADMIN</code>, hands them everything.</> },
  "ebpf": { Component: EbpfSection, title: <>eBPF &amp; the verifier — safe kernel programs</>, sub: <>eBPF runs <em>your</em> code inside the kernel — on the NIC (<strong>XDP</strong>), on any function (<strong>kprobes</strong>), on security hooks (<strong>LSM</strong>). That’s only sane because a static <strong>verifier</strong> proves each program safe before loading it: it must terminate (bounded loops), never read out of bounds, stay under a complexity limit, and call only allowed helpers. Write an XDP program that drops a DDoS flood, then break a property and watch the verifier reject it — so the filter never loads and the flood gets through.</> },
