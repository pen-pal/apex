...
import { StackCanarySection } from '../StackCanarySection';
import { CfiSection } from '../CfiSection';
...
import { DiffPrivSection } from '../DiffPrivSection';
...
export const chunk0: Record<string, SectionEntry> = {
  ...
  "vdf": { Component: VdfSection, title: <>A clock you can’t rush (verifiable delay function)</>, sub: <>A computation that provably takes a set amount of sequential time to evaluate but is fast to verify. Wesolowski’s VDF is y = x^(2ᵀ) mod N, computed by squaring x, T times in a row — each square needs the last, so the T-step delay can’t be parallelized, and there’s no shortcut without N’s discarded factorization. The prover attaches a short proof (random prime ℓ, π = x^⌊2ᵀ/ℓ⌋ mod N); the verifier checks π^ℓ · x^r ≡ y in two exponentiations. Verified in node: the sequential result equals x^(2ᵀ) and the proof verifies over 400 trials. Powers unbiasable randomness beacons (Chia, Ethereum). Step the squaring chain, then verify.</> },
  ...
  "stackcanary": { Component: StackCanarySection, title: <>How a stack canary catches the overflow</>, sub: <>A linear overflow has to write through everything between the buffer and the return address — so the compiler plants a random secret (the canary, from %fs:0x28) in the way. Reach the return address and you smash the canary; the function epilogue checks it before ret and calls __stack_chk_fail on a mismatch, aborting before your hijacked address is used. Overflow blind and it aborts; leak the canary, write it back, and the check passes. A canary is a check, not a wall.</> },
  "cfi": { Component: CfiSection, title: <>How CFI and shadow stacks kill ROP</>, sub: <>The final, structural mitigation. Instead of guarding a secret you can leak, control-flow integrity constrains where control may go: a shadow stack (Intel CET) keeps a hardware-protected second copy of every return address and compares it on ret, so an overflow-corrupted return faults (#CP) before the gadget runs — killing ROP; forward-edge IBT makes indirect branches land only on endbr64 targets. Fire a ROP chain with the shadow stack on and off. The arc's lesson: no single mitigation is a wall — security is layers.</> },
  ...
  "diffpriv": { Component: DiffPrivSection, title: <>Publishing data without leaking you (differential privacy)</>, sub: <>Release statistics about a group while provably hiding every individual. The Laplace mechanism answers a numeric query f(D) as f(D) + noise of scale Δ/ε — Δ is one person’s max influence (1 for a count), ε is the privacy budget. The guarantee (ε-DP): for two datasets differing by one person, the noisy outputs differ in probability by at most e^ε, so no observer can tell whether you’re in the data. Verified in node: that ratio is exactly e^ε (tight for ε=0.5/1/2), the mechanism is unbiased, noise std = √2·Δ/ε, and budgets compose. Slide ε and watch the two neighboring-dataset curves overlap or separate.</> },
