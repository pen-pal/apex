...
import { GraceJoin } from '../GraceJoinSection';
...
import { DeBruijnSection } from '../DeBruijnSection';
...
import { EevdfSection } from '../EevdfSection';
...
export const chunk4: Record<string, SectionEntry> = {
  ...
  "gracejoin": { Component: GraceJoin, title: <>How to join tables bigger than RAM (grace hash join)</>, sub: <>An in-memory hash join needs one table to fit in RAM. When both are far bigger, the grace hash join first partitions BOTH tables by hash(join key) into buckets — so matching keys always land in the same partition pair — spilling them to disk, then joins each small partition-pair in memory (build a hash table on R_i, probe with S_i). Verified in node: the output equals a nested-loop join exactly over 400 random joins, in O(|R|+|S|) I/O rather than O(|R|·|S|). Step through the partition and join phases.</> },
  ...
  "debruijn": { Component: DeBruijnSection, title: <>How genomes are assembled (de Bruijn graphs)</>, sub: <>A sequencer reads millions of short overlapping fragments; reassembling the original is genome assembly. Shred into k-mers, make each (k−1)-overlap a node and each k-mer a directed edge, and reassembly becomes an Eulerian path — walk every edge once (Hierholzer) and the sequence spells back out. Verified: distinct k-mers reconstruct the original exactly, each k-mer used once; but repeated (k−1)-mers branch the graph so several paths are valid (ambiguous) — the real difficulty, fixed by longer reads / larger k. Slide k and watch it snap between exact and ambiguous.</> },
  ...
  "eevdf": { Component: EevdfSection, title: <>How Linux schedules now (EEVDF)</>, sub: <>The scheduler that replaced CFS in Linux 6.6. Keep CFS’s weighted virtual runtime, but add two rules: a task is eligible only if its lag (V − v, the CPU it’s owed) is ≥ 0 — a task that ran ahead is benched — and among eligible tasks the one with the earliest virtual deadline (v + slice/weight) runs, bounding latency. Verified in node: the dispatched task is always eligible with the minimum deadline, Σ w·lag = 0, and long-run shares equal the weight fractions (1:2:4 → 14.3/28.6/57.1%). Step the scheduler and watch the lag bars and shares.</> },
