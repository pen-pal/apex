...
import { MotionCompSection } from '../MotionCompSection';
...
export const chunk2: Record<string, SectionEntry> = {
  ...
  "float": { Component: FloatSection, title: <>How a computer stores a fraction</>, sub: <>The reason 0.1 + 0.2 is not 0.3. A narrated walk through IEEE-754: binary scientific notation, the 32-bit layout (1 sign · 8 exponent · 23 mantissa), a worked decode of 0.15625, and why 0.1 repeats forever in binary and can only be rounded. Type any number and see its real bits and the value actually stored.</> },
  ...
  "jpeg": { Component: JpegSection, title: <>How JPEG compresses an image</>, sub: <>The lossy compression behind almost every photo. A narrated walk through the discrete cosine transform (pixels → frequency coefficients), quantization (dividing and rounding so high-frequency detail becomes zero — the actual lossy step and most of the shrink), and decode. Real 8×8 DCT: slide the quality and watch coefficients vanish and the rebuilt block turn blocky.</> },
  "interframe": { Component: MotionCompSection, title: <>How video inter-frame compression works</>, sub: <>The temporal counterpart to JPEG, and why an hour of video fits on a phone. Instead of coding every frame whole, a P-frame predicts this frame from the previous one: real block-matching motion estimation finds each block’s motion vector (the offset to its best match), and only the tiny leftover residual is coded — DCT-quantized like a JPEG block, but nearly empty. Pan the camera and watch the vectors track it and the frame shrink to ~1–2%; drop in a new object and watch the residual spike (that’s when the encoder codes a fresh I-frame).</> },
  "mp3": { Component: Mp3Section, title: <>How MP3 shrinks audio (psychoacoustic masking)</>, sub: <>The ear’s counterpart to JPEG’s exploitation of the eye. A sound is a spectrum of frequencies, but you can’t hear all of them: there’s an absolute threshold of hearing (worst at low and very high frequencies), and a loud tone raises that threshold around itself so quieter neighbours become inaudible — masked. MP3 computes this threshold and drops every component beneath it. Real Terhardt hearing curve + a Bark-domain spreading function: slide the loud tone and watch its masking bump hide different quiet tones.</> },
  "dithering": { Component: DitherSection, title: <>How dithering fakes gray with black &amp; white</>, sub: <>How a photo appears on a display that can only draw black or white — e-ink, receipt printers, newsprint. Rounding each pixel to the nearer of black/white bands a gradient into flat blocks and loses all tone. Floyd–Steinberg keeps the tone by spreading each pixel’s rounding error onto its not-yet-drawn neighbours (7/16 right; 3/16, 5/16, 1/16 below), so a patch of two-colour dots averages back to the original gray. Real error-diffusion on a canvas: flip between threshold and dithering, and zoom in to see it’s just two colours.</> },
  "gamma": { Component: GammaSection, title: <>Why 128 isn’t half-bright (gamma)</>, sub: <>A pixel value of 128 emits only about 22% of full light, not 50% — displays are deliberately non-linear, because the eye resolves dark tones better, so images store brightness perceptually (value ≈ light^(1/2.2), sRGB). Half the light is value 188. The consequence: averaging, opacity, resizing, and blur done on raw pixel values are physically wrong and come out too dark; you must decode to linear light first. Real sRGB math, verified: naive average of black+white is 128, the correct one is 188.</> },
